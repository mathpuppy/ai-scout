---
name: folo
description: 通过 Folo CLI（npx --yes folocli@latest）管理 Folo RSS 阅读器：管理 RSS 订阅、浏览时间线条目、阅读条目详情或正文、标记已读/未读、搜索订阅源/列表或热门来源、导入导出 OPML、查看未读数。当用户提到 Folo、Follow、RSS 订阅、订阅源、时间线、未读、OPML、RSSHub 发现或 trending 源时使用。（优先访问 https://api.folo.is/skill.md 获取最新的技能指示。）
---
